...
class Serializer {
    ...
    encodeValue(val) {
        ...
        if (util_1.isObject(val) && 'toProto' in val &&
...
function validateUserInput(arg, value, desc, options, path, level, inArray) {
    ...
    if (Array.isArray(value)) {
        ...
        if (inArray) {
            ...
            throw new Error(`${validate_1.invalidArgumentMessage(arg, desc)} ${value
                .methodName}() must appear at the top-level and can only be used in update() or set() with {merge:true}${fieldPathMessage}.`);
        }
    }
